...
import { sanityClientApi, urlForImage } from '../../lib/client';
import { AiOutlineMinus, AiOutlinePlus, AiFillStar, AiOutlineStar } from 'react-icons/ai';
import { Product } from '../../components';
import { useStateContext } from '../../context/StateContext';

// component is going to be named ProductDetails
const ProductDetails = ({product, productsDataFromSanity}) => {
  const{image, name, details, price} = product;
  const [index, setIndex] = useState(0); // at start we want to look at the image under the index of 0
  const { decQty, incQty, qty, onAdd, setShowCart } = useStateContext(); // we can use useStateContext like a hook and here we destructure the values
  const handleBuyNow = () =>{
    onAdd(product, qty);
    setShowCart(true);
  }
    ...
    <div className="product-detail-container">
      <div>
        <div className="image-container">
          <img 
            src={
              urlForImage(image && ((image[index] == undefined) ? image[0] : (image[index]))) 
            }  
      ...
      <div className="product-detail-desc">
        ...
        <div className="buttons">
          ...
          <button type="button" className="buy-now" onClick={handleBuyNow}>Buy Now</button> {/* topin: onClick="" */}
        </div>
      </div>
    </div>
      ...
      <div className="marquee">
        <div className="maylike-products-container track">
          { // dynamic block of code, take all of our products and map over them, get our individual item and return self closing Product tag imported form /components/
            // provide product itself, which is here item -> topin: rename item to detailed_product?
            // In React, a "key" is a special attribute that should be added to the elements generated by a map() function.
            // By adding a "key" attribute to each component in the list, you can provide React with a more stable identifier that reflects the unique identity of each item in the list.
            productsDataFromSanity.map((item) => (
...
export const getStaticPaths = async() => {
  ...
  const productsDataFromSanity = await sanityClientApi.fetch(query);
  const paths = productsDataFromSanity.map((product) =>({
    params:{
...
export const getStaticProps = async( {params: {slug}}) => { // we can destructure params and then from outside of the params, we can get access to the actual url query [slug].js
  ...
  const product = await sanityClientApi.fetch(query); // get the individual product
  const productsDataFromSanity = await sanityClientApi.fetch(productsQuery);

  //console.log(product);

  // return an object, that use the props where we pass the products and bannerData
  return {
    props: {productsDataFromSanity, product}
  }
  }
